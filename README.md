...
**Every grid opens in time order**, where it has a clock to open in: the column that makes a row an event (`timestamp_utc`, or a finding's `first_utc`) sorts ascending the moment the table is opened, because evidence is read forwards and the order rows happened to be appended in is an accident of how many files the artifact was split across. Tables where a time is an attribute of a standing thing — `USERS`, `SUID_SGID`, `PROCESS_TREE` — keep their own order, which for a tree or an inventory is the meaningful one. Click any heading to sort by it, click again to reverse.

**And a column sorts as what it holds.** Numbers sort as numbers, so 9 comes before 10 rather than after 1; timestamps sort chronologically, including the raw syslog stamps where `Apr` alphabetically precedes `Mar`; everything else sorts as text, case-insensitively, with any numbers in it ahead of the words. Empty cells go to the bottom whichever way the arrow points — a column sorted to bring the largest to the top should not bring forty blanks instead — and rows that tie keep the order they were already in, so a second sort refines the first rather than scrambling it.
