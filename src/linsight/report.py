...
def write_html(tri, path, opts, tb=None):
    esc = htmllib.escape
    ...
    tables = list(getattr(tb, "tables", None) or [])
    if tables:
        rows = sum(len(t) for t in tables)
        where = []
        for flag, what in ((getattr(opts, "export", ""), "browser.html and csv/ under %s"),
                           (getattr(opts, "tables_html", ""), "%s"),
                           (getattr(opts, "csv_dir", ""), "%s")):
            if flag:
                where.append(what % flag)
        parts.append("<h2>Artifact tables (%d, %s rows)</h2>"
                     % (len(tables), format(rows, ",")))
        parts.append("<div class='meta'><div>This report carries the findings. "
                     "The artifact tables are a browsable grid and live "
                     "outside it%s.</div></div>"
                     % ((" - in " + "; ".join(esc(w) for w in where)) if where
                        else ", and were not exported by this run: add "
                             "<code>--export DIR</code> for the browser page "
                             "and the CSVs"))
        parts.append("<table><tr><th>table</th><th>rows</th>"
                     "<th>category</th><th>what it holds</th></tr>")
        for t in sorted(tables, key=lambda x: (x.category or "", x.name)):
            parts.append("<tr><td><code>%s</code></td><td>%s</td><td>%s</td>"
                         "<td>%s</td></tr>"
                         % (esc(t.name), format(len(t), ","),
                            esc(t.category or ""), esc(trunc(t.title, 90))))
        parts.append("</table>")
